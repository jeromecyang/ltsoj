...
date: 2022-02-26
permalink: ukraine
image: https://ichef.bbci.co.uk/news/976/cpsprodpb/1176F/production/_123453517_gettyimages-1373101584-594x594.jpg
---
...
<script>
const countries = [0, 10781, 10782, 10783, 10784, 10785, 10786, 12002];
const directory = {
  0: {name_zh: '總計', name: '', flag: '', color: '#888888'},
  10781: {name_zh: '波蘭', name: 'Poland', flag: '🇵🇱', color: '#003f5c', population: 38116000},
  10782: {name_zh: '羅馬尼亞', name: 'Romania', flag: '🇷🇴', color: '#ff6361', population: 19186201},
  10783: {name_zh: '匈牙利', name: 'Hungary', flag: '🇭🇺', color: '#bc5090', population: 9689000},
  10784: {name_zh: '摩爾多瓦', name: 'Republic of Moldova', flag: '🇲🇩', color: '#ffa600', population: 2597100},
  10785: {name_zh: '斯洛伐克', name: 'Slovakia', flag: '🇸🇰', color: '#58508d', population: 5452025},
  10786: {name_zh: '白羅斯', name: 'Belarus', flag: '🇧🇾', color: '#888888'},
  12002: {name_zh: '俄羅斯', name: 'Russian Federation', flag: '🇷🇺', color: '#888888'},
};

const dataToMap = ({ data }) => data.reduce((agg, { geomaster_id, individuals, date }) => ({ ...agg, [geomaster_id]: { value: Number(individuals), date } }), {});

Promise.all([
  fetch('https://data.unhcr.org/population/?widget_id=285660&sv_id=54&population_group=5460').then(r => r.json()).then(dataToMap),
  fetch('https://data.unhcr.org/population/get/sublocation?widget_id=336969&sv_id=54&population_group=5460').then(r => r.json()).then(dataToMap),
  fetch('https://data.unhcr.org/population/get/sublocation?widget_id=336970&sv_id=54&population_group=5472').then(r => r.json()).then(dataToMap),
  fetch('https://data.unhcr.org/population/get/sublocation?widget_id=336974&sv_id=54&population_group=5480').then(r => r.json()).then(dataToMap),
]).then(([refugeeTotal, refugeeMap, returneeMap, registrantMap]) => {
  refugeeMap[0] = refugeeTotal[undefined];
  const rows = countries.sort((a, b) => refugeeMap[b].value - refugeeMap[a].value).map(id => {
    const { name_zh, name, color, flag, population } = directory[id];
    const refugee = refugeeMap[id];
    const returnee = returneeMap[id];
    const registrant = registrantMap[id];
    return `<div style="color: #ffffff; background-color: ${color}; margin: 10px 0; padding: 10px">
      <div>${name_zh} ${name} ${flag}</div>
      <div style="font-size: 2rem; line-height: 2rem; margin-top: 0.5rem">
        ${refugee.value.toLocaleString()}
        <span style="font-size: 0.8rem">
          (${refugee.date})
          ${returnee ? `(${returnee.value.toLocaleString()} 返回烏克蘭)` : ``}
        </span>
      </div>
      ${population && registrant ? `<div style="margin-top: 0.4rem; font-size: 0.8rem">平均每 <span style="font-size: 1.2rem">${(population / registrant.value).toFixed(0)}</span> 人支持一位註冊難民 (註冊人數 ${registrant.value.toLocaleString()} ，總人口 ${population.toLocaleString()})</div>` : ``}
    </div>`;
  });
...
<img src="https://i.imgur.com/BFJA8ngl.jpg" style="width: 300px">
[俄羅斯與他的快樂夥伴們，那些傻傻分不清楚的東方/共產陣營組織](https://ltsoj.com/map/eastern-bloc)

<img src="https://i.imgur.com/rwPR46Hl.jpg" style="width: 300px">
[從波羅的海、黑海到亞得里亞海都略懂：斯拉夫語族](https://ltsoj.com/map/slavic)

<img src="https://i.imgur.com/Dd2T18dl.jpg" style="width: 300px">
[「不顧基輔反對」從烏克蘭獨立的兩小國：盧甘斯克、頓內次克](https://ltsoj.com/map/east-ukraine)

<img src="https://i.imgur.com/YH3tQnfl.jpg" style="width: 300px">
[克里米亞半島](https://ltsoj.com/map/crimea)

<img src="https://i.imgur.com/I2CxNiLl.jpg" style="width: 300px">
